function renderOpeningTexts() {
    var addTextInterval = setInterval(function () {
        ...
        var textnode2 = document.createTextNode("במעבר בין תחנות המידע באתר פרטיול, תקבלו כלים לשלוט טוב יותר על המידע האישי והרגיש שלכם ברשת.");
...
function renderHomeLogos() {
    var renderLogosInterval = setInterval(function () {
        ...
        hitLogo.classList.add("hp-hit-logo");

        var mpLogo = document.createElement('img');
        mpLogo.src = 'assets/mp.png';
        mpLogo.classList.add("hp-mp-logo");
        
        logoDiv.appendChild(hitLogo);
        logoDiv.appendChild(mpLogo);

        clearInterval(renderLogosInterval);
    }, 500)
}
